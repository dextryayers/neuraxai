...
import { Send, Paperclip, Loader2, User, Copy, Check, Menu, Globe, Brain, Sparkles, BadgeCheck, Clock } from 'lucide-react';
...
const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  // Generate random stars on mount - EVENLY SPREAD
  useEffect(() => {
    // Increased to 400 for full screen coverage
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-transparent relative font-sans overflow-hidden">
      
      {/* Header (Glass & Minimal) */}
      <header className="flex-none flex items-center justify-between px-6 py-4 bg-transparent border-b border-white/5 z-20">
        <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
                ...
                <div className="glass-panel px-3 py-1 rounded-full border border-white/10">
                    <h2 className="text-xs font-bold text-slate-200 tracking-wide flex items-center gap-1.5">
                        NeURAX AI
      ...
      {/* Messages Area */}
      <div 
        className="flex-1 overflow-y-auto overflow-x-hidden px-4 py-6 custom-scrollbar scroll-smooth relative" 
        style={{ overscrollBehaviorY: 'contain' }}
      >
        {messages.length === 0 && (
          <>
            {/* --- GLOBAL ANIMATED STAR FIELD (FIXED TO VIEWPORT FOR EVEN SPREAD) --- */}
            <div className="fixed inset-0 pointer-events-none z-0 overflow-hidden">
                {stars.map((star) => (
                    <div 
                        key={star.id}
                        className={`absolute bg-white rounded-full animate-twinkle ${star.size} shadow-[0_0_4px_white]`}
                        style={{
                            top: star.top,
                            left: star.left,
            ...
            {/* Changed from h-[70vh] to min-h-full to allow scrolling on small screens if needed */}
            <div className="flex flex-col items-center justify-center min-h-full py-10 text-slate-400 animate-in fade-in zoom-in duration-700 relative z-10">
                
                {/* Robot Assembly - RESPONSIVE */}
                <div className="relative mb-16 group scale-100 md:scale-125 origin-center">

                    {/* --- ADVANCED HOLOGRAPHIC STAGE (Brighter & More Visible) --- */}
                    {/* Uses responsive vw units for width to maintain fixed layout on mobile */}
                    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[80vw] max-w-[600px] h-[80vw] max-h-[600px] pointer-events-none z-0">
                        
                        {/* 1. Deep Core Glow (Pulse) - Increased Opacity */}
                        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[60%] h-[60%] bg-blue-600/40 rounded-full blur-[80px] animate-pulse-slow mix-blend-screen"></div>
                        
                        {/* 2. Rotating Cyber Rings (Floor Perspective) */}
                        
                        {/* Outer Ring (Slow Blue) - More Visible Border & Shadow */}
                        <div className="absolute top-[55%] left-1/2 -translate-x-1/2 w-[90%] h-[90%] border border-blue-400/30 rounded-full scale-y-[0.25] animate-[spin_20s_linear_infinite] shadow-[0_0_30px_rgba(59,130,246,0.2)]"></div>
                        
                        {/* Middle Ring (Reverse Purple Dashed) - Stronger Color */}
                        <div className="absolute top-[58%] left-1/2 -translate-x-1/2 w-[70%] h-[70%] border border-dashed border-purple-400/40 rounded-full scale-y-[0.25] animate-[spin_15s_linear_infinite_reverse]"></div>
                        
                        {/* Inner Ring (Fast Cyan Dotted) - Glowing Cyan */}
                        <div className="absolute top-[60%] left-1/2 -translate-x-1/2 w-[50%] h-[50%] border-2 border-dotted border-cyan-400/50 rounded-full scale-y-[0.25] animate-[spin_10s_linear_infinite] shadow-[0_0_15px_rgba(34,211,238,0.4)]"></div>

                        {/* 3. Vertical Light Column (Rising Energy) - Brighter Gradient */}
                        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-[40%] w-[30%] h-[80%] bg-gradient-to-t from-blue-500/30 via-purple-500/10 to-transparent blur-2xl"></div>
                    </div>

                    <div className="flex flex-col items-center relative z-10">
                        
                        {/* --- SPEECH BUBBLE POPUP --- */}
                        <div className="absolute -top-24 -right-24 z-[100] animate-pop-bounce delay-150 opacity-0 origin-bottom-left hidden md:block">
                        ...
                        {/* Mobile optimized bubble */}
                        <div className="absolute -top-20 z-[100] animate-pop-bounce delay-150 opacity-0 origin-bottom md:hidden">
                            <div className="bg-white text-slate-900 px-3 py-2 rounded-xl shadow-[0_0_15px_rgba(255,255,255,0.4)] border border-blue-200 relative">
                            <span className="text-[10px] font-bold leading-tight block">Ahoy! 👋</span>
                            </div>
                        </div>

                        {/* Base Emitter (Direct contact) - Brighter */}
                        <div className="absolute -bottom-12 w-32 h-8 bg-blue-500/50 rounded-[100%] blur-md z-0 animate-pulse shadow-[0_0_20px_rgba(59,130,246,0.6)]"></div>
                        <div className="absolute -bottom-12 w-20 h-2 bg-white/50 rounded-[100%] blur-[2px] z-0"></div>
                        ...
                        <div className="relative mt-[-2px] flex justify-center"> 
                            {/* Left Arm (Static) - Layer z-10 */}
                            <div className="absolute top-[8px] right-[76px] z-10 origin-top transform rotate-[5deg]">
                            ...
                            {/* Right Arm (Waving) - Layer z-10 - PRECISELY FIXED FOR HIGH WAVE */}
                            <div className="absolute top-[8px] -right-[14px] z-10 origin-top animate-wave">
                ...
                <div className="text-center space-y-3 relative px-8 py-2">
                    {/* Blink Blink Sparkles around Text */}
                    ...
                    <p className="text-sm text-slate-300 font-medium">Advanced Multi-Model Intelligence. Ready to assist.</p>
                </div>
            </div>
          </>
        )}
